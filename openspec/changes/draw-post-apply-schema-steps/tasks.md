...
- [x] 4.7 Verify the VS Code webview rendering of the derived edge against the real host, since the host injects its own stylesheet and this class of problem cannot reproduce in a browser
  - Confirmed by the maintainer against the real webview (extension built from this branch and installed as `kewang.spek-vscode@1.12.0`). This session is the *remote* half of an SSH session, so the Electron app runs on their machine and could not be driven from here — the check had to be human.
  ...
  - The two chips in the derived-order callout (`requires`, `openspec`) render as chips, matching the pre-existing `plan` and `verify.md` chips on the same panel, rather than picking up the host's `code` styling.
